...
void main()
{
    outputColor = color * texture(in_fontTexture, texCoord);
        ...
        {
            int viewportWidth = Graphics.GetScreenWidth();
            int viewportHeight = Graphics.GetScreenHeight();
            ImGuiIOPtr io = ImGui.GetIO();
            io.DisplaySize = new Vector2(
                viewportWidth / _scaleFactor.X,
                viewportHeight / _scaleFactor.Y);
        ...
        public void OnPressChar(uint codepoint)
        {
            PressedChars.Add((char)codepoint);
        }

        public void OnMouseScroll(float offsetX, float offsetY)
        {
            ImGuiIOPtr io = ImGui.GetIO();
        ...
        private void RenderImDrawData(ImDrawDataPtr draw_data)
        {
            ...
            for (int n = 0; n < draw_data.CmdListsCount; n++)
            {
                ...
                for (int cmd_i = 0; cmd_i < cmd_list.CmdBuffer.Size; cmd_i++)
                {
                    ...
                    {
                        int windowHeight = Graphics.GetScreenHeight();
