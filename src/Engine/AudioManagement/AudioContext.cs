...
{   
    ...
    public sealed class AudioContext : IDisposable
    {
        ...
        internal void Add(AudioListener listener)
        {
            for(int i = 0; i < listeners.Count; i++)
            {
                if(listeners[i].GetHashCode() == listener.GetHashCode())
                    return;
            }
        ...
        internal void Remove(AudioListener listener)
        {
            int index = -1;
        ...
        internal void Add(AudioClip clip)
        {
            for(int i = 0; i < clips.Count; i++)
            {
                if(clips[i].GetHashCode() == clip.GetHashCode())
                    return;
            }
        ...
        internal void Remove(AudioClip clip)
        {
            int index = -1;
        ...
        public void Dispose()
        {
            // Calling Dispose removes item from the list, so we make a copy first
            var sourcesCopy = sources.ToArray();
            var clipsCopy = clips.ToArray();

            for(int i = 0; i < sourcesCopy.Length; i++)
                sourcesCopy[i]?.Dispose();

            for(int i = 0; i < clipsCopy.Length; i++)
                clipsCopy[i]?.Dispose();
